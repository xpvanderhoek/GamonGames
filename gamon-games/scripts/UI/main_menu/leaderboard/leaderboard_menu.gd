...
func _build_tabs() -> void:
	var normal_style := _make_stylebox(C_TAB_BG, 10, 5)
	var hover_style  := _make_stylebox(C_TAB_HOVER, 10, 5)
	var active_style := _make_stylebox(C_TAB_ACTIVE, 10, 5)
	...
	for stat in STATS_TO_SHOW:
		...
		btn.pressed.connect(_on_tab_pressed.bind(stat, btn))
...
func _show_leaderboard(stat: String) -> void:
	current_stat = stat
	stat_label.text = _format_stat_name(stat).to_upper()
	...
	sorted.sort_custom(func(a, b):
		return a["stats"].get(stat, 0.0) > b["stats"].get(stat, 0.0))
	...
	for i in range(sorted.size()):
		...
		var val: float = profile["stats"].get(stat, 0.0)
		...
		row.add_child(_make_cell(_format_value(stat, val), COL_VALUE,
