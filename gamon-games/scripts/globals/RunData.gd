...
func add_exp(amount: int) -> void:
	current_exp += amount
	while (
		current_level < EXP_PER_LEVEL.size() - 1 and current_exp >= EXP_PER_LEVEL[current_level + 1]
	):
		level_up()
...
func get_level_progress() -> float:
	...
	var next_requirement = (
		EXP_PER_LEVEL[current_level + 1]
		if current_level + 1 < EXP_PER_LEVEL.size()
		else EXP_PER_LEVEL[current_level]
	)
	var progress = (
		float(current_exp - current_requirement) / float(next_requirement - current_requirement)
	)
