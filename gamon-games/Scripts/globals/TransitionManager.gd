...
enum TransitionType { FADE = 0, SLIDE = 1, WIPE = 2 }
...
func transition_combat(
	enter: bool,
	combat_path: String = "",
	transition_type: int = TransitionType.FADE,
	enemy: Node = null
) -> void:
	_setup_transition(transition_type)
	await get_tree().process_frame
	...
	if enter and game.has_method("enter_combat"):
		...
		push_warning(
			(
				"transition_combat: game scene missing expected method. enter="
				+ str(enter)
				+ " scene="
				+ str(game)
			)
